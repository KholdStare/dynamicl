...
namespace DynamiCL
{
    ...
    template <typename PixType, size_t N>
    class HostImageBlah : private HostImageView<PixType, N>
    {
        static_assert( N >= 1, "An image has to have at least one dimension." );
        // TODO: add assert once availble in gcc
        //static_assert( std::is_trivially_copyable<PixType>,
        ...
        HostImageBlah(std::array<size_t, N> const& dims, PixType* data)
            : view_type(dims, data)
        { }
        ...
        friend class HostImageBlah<PixType, N-1>;

    public:

        HostImageBlah(std::array<size_t, N> const& dims)
            : view_type(dims, new PixType[view_type::multDims(dims)])
        { }

        HostImageBlah(size_t width, size_t height)
            : view_type({{width, height}}, new PixType[width*height])
        { }

        HostImageBlah(size_t width, size_t height, size_t depth)
         ...
         * @note input images are deallocated.
         */
        HostImageBlah(std::vector<HostImageBlah<PixType, N-1>> const& subimages);

        // TODO: documentation
        HostImageBlah(HostImageBlah<PixType, N+1>&& toBeCollapsed)
        {
        ...
        // disable copying because expensive
        HostImageBlah(HostImageBlah const& other) = delete;
        HostImageBlah& operator =(HostImageBlah const& other) = delete;

        // move constructor
        HostImageBlah(HostImageBlah&& other)
            : view_type(std::move(other))
        { }

        // move assignment
        HostImageBlah& operator =(HostImageBlah&& other)
        {
            return view_type::operator=(std::move(other));
            //std::copy_n(other.dims_.begin(), N, dims_.begin());
            //pixArray_ = other.pixArray_;
         ...
         */
        // TODO: update comment
        HostImageBlah<PixType, N-1> collapseDimension()
        {
            // create colapsed dimensions
            std::array<size_t, N-1> dims;
            std::copy_n(view().dimensions().begin(), N-1, dims.begin());
            dims[N-2] *= view().dimensions()[N-1];

            // transfer image data
            return HostImageBlah<PixType, N-1>(dims, releaseData());
        }
    };
    ...
    template <typename PixType, size_t N>
    HostImageBlah<PixType, N>::HostImageBlah(std::vector<HostImageBlah<PixType, N-1>> const& subimages)
    {
        assert(subimages.size() > 0);
        ...
        typedef HostImageBlah<PixType, N-1> subimage_type;
        for(subimage_type const& subimage : subimages)
        {
            // ensure all dimensions match
     ...
     * a collapsing of two- 3x4x5 -> 3x20
     */
    // TODO: reimplement as member. make releaseData() private members
    template <typename PixType, size_t N>
